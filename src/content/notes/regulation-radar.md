...
The <a href="/notes/better-call-saullm" target="_blank">previous experiment</a> in this series asked six models to classify ambiguous legal documents. They disagreed extensively – but there was no ground truth to grade against. This time, the librarians provided one. Sort of. The EuroVoc classifications are not a perfect answer key. The librarians assign granular concept descriptors, which get rolled up to the 21 top-level domains through a deterministic mapping. A regulation about agricultural subsidies in the context of trade negotiations might be tagged "Agriculture" by one route and "Trade" by another, and both would be defensible. But it is structured, consistent, and publicly available – considerably better than having no benchmark at all.
